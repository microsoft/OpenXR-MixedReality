...
tbool genTangSpace(const SMikkTSpaceContext * pContext, const float fAngularThreshold)
{
	...
	{
#pragma prefast(push)
#pragma prefast(disable: 6386) // PREfast is confused about the writeable size
		...
		else
		{
...

			{
				...
				else
				{
					...
					bQuadDiagIs_02 = distSQ_13_2<distSQ_02_2 ? TFALSE : TTRUE;
				}
...
static void DegenPrologue(STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris)
{
	...
	{
		tbool bIsGood = (pTriInfos[t].iFlag&MARK_DEGENERATE)==0 ? TTRUE : TFALSE;
		if (bIsGood)
		{
		...
		else
		{
			...
			{
				bIsGood = (pTriInfos[iNextGoodTriangleSearchIndex].iFlag&MARK_DEGENERATE)==0 ? TTRUE : TFALSE;
